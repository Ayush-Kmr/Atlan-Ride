...
export class CityComponent implements OnInit {
  ...
  initMap() {
    ...
    this.polygon = new google.maps.Polygon({
      paths: [], // Empty initially, can be updated when a zone is drawn
      strokeColor: "#FF0000",
      strokeOpacity: 0.8,
      strokeWeight: 2,
      fillColor: "#FF0000",
    ...
    // Add the polygon to the map (optional, can be done after a zone is drawn)
  ...
  onSelected(id: any) {
    ...
    console.log(id);
    const selectedCountryObj = this.countryData.find(
      (obj: any) => obj._id === id
    );
  ...
  checkZone_AddCity() {
    if (this.cityForm.value.countryname != "") {
      ...
      const input = document.getElementById("inputCity") as HTMLInputElement;
      console.log(this.polygon, location);
